...
export default {
  ...
  computed: {
    ...
    nodes () {
      ...
      if (this.isRoot) {
        ...
        if (!this.storeNodes.length) {
          this.mergeStoreNodes(this.getNodes(nodeModels))
        }
        // if (this.bakNodes.length <= 0) {
        //   this.bakNodes = this.getNodes(nodeModels)
        // }
        // console.log('if-----  this.bakNodes=', this.bakNodes)
        return this.getNodes(nodeModels)
      }
      // console.log('else----  this.bakNodes=', this.bakNodes)
  ...
  methods: {
    // 合并nodes值
    mergeStoreNodes (thisNodes) {
      console.log('合并nodes值')
      this.$store.dispatch('updateNodes', thisNodes)
    },
    ...
    getNode (
      ...
      const node = {
        ...
        isDraggable,
        isSelectable,
        isClickContextMenu: !!nodeModel.isClickContextMenu,
    ...
    emitNodeContextmenu (node, index, event) {
      // console.log('emitNodeContextmenu---')
      this.ergodicNodes(this.storeNodes) // 设置所有节点不显示右键样式
      this.updateNode(node.path, { isClickContextMenu: true }) // 在当前点击的节点显示右键样式
      this.getRoot().$emit('nodecontextmenu', node, index, event)
    },
    ...
    select (path, addToSelection = false, event = null) {
      ...
      this.traverse((node, nodeModel) => {
        ...
        nodeModel.isClickContextMenu = false // 不显示节点的右键样式效果
    ...
    // 设置选中节点右键菜单时的样式
    // setClickContextMenu (path, addToSelection = false, event = null) {
    //   const selectedNode = this.getNode(path)
    //   if (!selectedNode) return null
    //   const newNodes = this.copy(this.currentValue)
    //   this.traverse((node, nodeModel) => {
    //     if (node.pathStr === selectedNode.pathStr || node.pathStr === this.lastSelectedNode.pathStr) {
    //       nodeModel.isClickContextMenu = node.isClickContextMenu
    //     } else if (node.pathStr === selectedNode.pathStr) {
    //       nodeModel.isClickContextMenu = node.isClickContextMenu
    //     } else if (!addToSelection) {
    //       if (nodeModel.isClickContextMenu) nodeModel.isClickContextMenu = false
    //     }
    //     // if (shiftSelectionMode) {
    //     //   if (node.pathStr === selectedNode.pathStr || node.pathStr === this.lastSelectedNode.pathStr) {
    //     //     nodeModel.isSelected = node.isSelectable
    ...
    onMousemoveHandler (event) {
      ...
      const isDragging =
        this.isDragging || (this.mouseIsDown && (this.lastMousePos.x !== event.clientX || this.lastMousePos.y !== event.clientY))
